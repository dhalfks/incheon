...
public class Hotel {
	...
	private void initializeRooms() {
		for (int i = 0; i < rooms.length; i++) {
			for (int j = 0; j < rooms[i].length; j++) {
				rooms[i][j] = new Room(makeRoomNumber(i, j), 
										null, true, null, null);
			}
		}
	}
	...
	private void getRoomDetail(String roomNum) {
		...
		int floor = fr[0];
		int room = fr[1];
		System.out.println(rooms[floor][room]); // Room Object
		System.out.println(rooms[floor][room].getUser()); // User Object
	}
	...
	private void roomCheckOut(String roomNum) {
		...
		if (!rooms[floor][room].isEmpty()) {
			...
			System.out.println(roomNum + "호는 현재 빈 방! > 사용중인 방을 선택하세요!");
		}
