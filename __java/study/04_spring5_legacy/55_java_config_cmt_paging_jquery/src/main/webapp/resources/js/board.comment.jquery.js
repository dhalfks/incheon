function postCommentToServer(cmtData) {
    let result = "0";
    $.ajax({
        type: "post",
        url: "/bcomment/post",
...
$("#cmtPostBtn").on('click',()=>{
    ...
    if (cmtText.val() == null || cmtText.val() =='') {
        alert('댓글 내용을 입력해주세요!');
        cmtText.focus();
        return false;
    } else {
...
function spreadCommentFromServer(bno, page){
    console.log(bno);
    $.ajax({
        type: "get",
        url: "/bcomment/"+bno+"/"+page,
        dataType: "json"
